...
const preparePjaxPwa = (app, pjaxPwaOptions) => {
  ...
  if (typeof app.locals.theme.icon192Url === 'undefined') {
    throw new Error('No app.locals.theme.icon192Url specified. Did you call prepareTheme() before prepearePjaxPwa? Did you forget to set THEME_ICON_192_URL?')
  }
  if (typeof app.locals.theme.publicFilesUrl === 'undefined') {
    throw new Error('No app.locals.theme.publicFilesUrl specified. Did you call prepareTheme() before prepearePjaxPwa? Did you forget to set THEME_PUBLIC_FILES_URL?')
  }
  ...
  const defaults = {
    ...
    icon192Url: app.locals.theme.icon192Url || path.join(__dirname, '..', 'public', 'icon192.png'),
...
const setupPjaxPwa = (app, config) => {
  ...
  const {
    ...
    withPjaxPwa,
    networkErrorUrl
  } = config || app.locals.pjaxPwa
  const { icon192Url } = config || app.locals.theme
